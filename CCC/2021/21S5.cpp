...
const int N = 15e4 + 10;
int n, m, X[N], Y[N], Z[N];
int lazy[N][20], key[N][20], tot = 1, cur[20], st[N << 2], ans[N];

inline void build(int rt, int l, int r){
    if(l == r){
        for(int i = 1;i<=16;i++){
            cur[i] += lazy[tot][i];
            if(cur[i]) ans[tot] = ans[tot] == 0 ? i : lcm(i, ans[tot]);
        }
        st[rt] = ans[tot++];
        return;
    }
...
inline int query(int rt, int l, int r, int x, int y){
    ...
    else return gcd(query(lc, l, mid, x, mid), query(rc, mid + 1, r, mid + 1, y));
}
