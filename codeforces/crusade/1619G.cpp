...
vector<bomb> a;
// unordered_map<int, vector<bomb> > hor, ver;

bool cmp(bomb a, bomb b) {return a.y < b.y;}
bool cmp2(bomb a, bomb b) {return a.x < b.x;}

int Find(int x) {return f[x] == x ? x : f[x] = Find(f[x]);}
void merge(int u, int v) {
    int fu = Find(u);
    int fv = Find(v);
    if(a[fu].t > a[fv].t) f[fu] = fv;
    else f[fv] = fu;
}
...
inline void solve(){
    int n = read(), k = read();
    iota(f + 1, f + n + 1, 1);
    a.clear();// hor.clear(), ver.clear();
    a.resize(1);
	gp_hash_table<int, vector<bomb> > hor({},{},{},{},{1<<13}); 
	gp_hash_table<int, vector<bomb> > ver({},{},{},{},{1<<13}); 
    for(int i = 1;i<=n;++i) {
    ...
    for(auto& cur : hor) {
        vector<bomb>& x = cur.sec;
        sort(all(x), cmp);
        for(int i = 0;i<x.size() - 1;++i) 
            if(abs(x[i].y - x[i + 1].y) <= k) merge(x[i].id, x[i + 1].id);
    }
    for(auto& cur : ver) {
        vector<bomb>& x = cur.sec;
        sort(all(x), cmp2);
        for(int i = 0;i<x.size() - 1;++i) 
            if(abs(x[i].x - x[i + 1].x) <= k) merge(x[i].id, x[i + 1].id);
    }
    sort(all(a));
    int t = -1;
    for(auto x : a) {
