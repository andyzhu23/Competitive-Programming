...
int n, m;
string s;
ull p[N][5] = {1}, base[5] = {13331}, h[N][30][5], mod = 998244353;
bool vis[26] = {0};

int tot = 1;

inline ull Hash(int l, int i, int len) {
    return (h[l + len - 1][i][0] + mod - h[l - 1][i][0] * p[len][0] % mod) % mod;
}

inline void solve() {
    for(int i = 0;i<=tot;++i) p[0][i] = 1;
    for(int i = 1;i<N;++i) 
        for(int k = 0;k<=tot;++k) p[i][k] = p[i - 1][k] * base[k] % mod;
    read(n), read(m);
    cin>>s;
    s = '@' + s;
    for(int i = 1;i<=n;++i) 
        for(int j = 1;j<=26;++j) 
            for(int k = 0;k<=tot;++k) 
                h[i][j][k] = h[i - 1][j][k] * base[k] % mod + (s[i] - 'a' + 1 == j);
    while(m--) {
        int l = read(), r = read(), len = read();
        vec<ull> a, b;
        for(int i = 1;i<=26;++i) a.pb(Hash(l, i, len));
        for(int i = 1;i<=26;++i) b.pb(Hash(r, i, len));
        sort(all(a)), sort(all(b));
        bool flag = 1;
        for(int i = 0;i<26;++i) flag &= a[i] == b[i];
        flag ? printf("YES\n") : printf("NO\n");
    }
}
