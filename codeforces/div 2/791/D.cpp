...
inline void solve(){
    ...
    while(lo <= hi) {
        ...
        flag = 0;
        mx = 0;
        for(int i = 1;i<=n;++i) if(a[i] <= mid) ckmax(mx, dfs(i));
        memset(low, 0, sizeof(low));
        memset(dfn, 0, sizeof(dfn));
        vis.reset();
        tot = 0;
        for(int i = 1;i<=n;++i) if(!dfn[i] && a[i] <= mid) tarjan(i);
        if(mx >= k || flag) ans = mid, hi = mid - 1;
        else lo = mid + 1;
    }
    print(ans, '\n');
}
