...
inline void solve(){
    ...
    while(q--) {
        ...
        if(opt == 1) {
            int u = read(), v = read();
            if(!r[u]) fr.update(u, 1);
            ++r[u];
            if(!c[v]) fc.update(v, 1);
            ++c[v];
        } else if(opt == 2) {
            int u = read(), v = read();
            --r[u];
            ...
            int u = fr.query(min(x, z), max(x, z));
            int v = fc.query(min(y, w), max(y, w));
            puts(u == abs(x - z) + 1 || v == abs(y - w) + 1 ? "Yes" : "No");
        }
    }
}
