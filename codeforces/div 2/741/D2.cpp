...
inline void solve() {
    ...
    um<int, vi> Map, Map2;
    for(int i = 1;i<=n;i++) {
        sum[i] = sum[i - 1] + (((i & 1) ^ (s[i - 1] == '-')) ? 1 : -1);
        ((i & 1) ^ (s[i - 1] == '-')) ? Map[sum[i]].pb(i) : Map2[sum[i]].pb(i);
    }
    for(int i = 1;i<=q;i++) {
        int l, r; cin>>l>>r;
        int len = r - l + 1;
        ...
        int tot = ((sum[r] - sum[l - 1] + 1) >> 1) + sum[l - 1];
        int ans = !Map[tot].empty() && Map[tot][Map[tot].size() - 1] >= l ? *lb(all(Map[tot]), l) : INF;
        if(ans <= r) {
            cout<<ans<<'\n';
            continue;
        }
        tot = ((sum[r] - sum[l - 1]) >> 1) + sum[l - 1];
        ans = !Map2[tot].empty() && Map2[tot][Map2[tot].size() - 1] >= l ? *lb(all(Map2[tot]), l) : INF;
        cout<<ans<<'\n';
    }
}
