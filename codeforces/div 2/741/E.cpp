...
inline void solve() {
    ...
    for(int i = 1;i<=n + 5;++i) {
        dp[i] = 0;
        for(int j = 1;j<=n + 5;++j) lps[i][j] = 0;
    }
    for(int i = n;i>0;--i) {
        for(int j = n;j>0;--j) {
            lps[i][j] = s[i - 1] == s[j - 1] ? lps[i + 1][j + 1] + 1 : 0;
        }
    }
    ll ans = 0;
    for(int i = n;i>0;--i) {
        dp[i] = n - i + 1;
        for(int j = i + 1;j<=n;++j) {
            if(s[i + lps[i][j] - 1] <= s[j + lps[i][j] - 1]) {
                dp[i] = max(dp[i], n - i + 1 + dp[j] - lps[i][j]);
            }    
        }
        ans = max(ans, dp[i]);
    }
