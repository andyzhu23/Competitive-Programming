...
inline void solve(){
    ...
    ll ans = 0;
    for(auto x : c) {
        sort(all(edge[x]), [&](pii& a, pii& b) {
            return dep[a.fir] + dep[a.sec] < dep[b.fir] + dep[b.sec];
        });
        // cout<<x<<'\n'<<flush;
        for(auto[u, v] : edge[x]) {
            remove(u, v);
            // cout<<u<<' '<<v<<' '<<st.query(1, 1, n, dfn[u])<<'\n'<<flush;
        }
        for(auto[u, v] : edge[x]) {
            ans += 1ll * sz[st.query(1, 1, n, dfn[u])] * sz[st.query(1, 1, n, dfn[v])];
        }
        for(int i = edge[x].size() - 1;~i;--i) {
            auto[u, v] = edge[x][i];
            merge(u, v);
        }
    }
