...
struct segtree {
    ...
    int queryAnd(int rt, int l, int r, int x, int y) {
        if(l == x && y == r) return a[rt];
        int mid = l + r >> 1;
        push_down(rt);
        if(y <= mid) return queryAnd(lc, l, mid, x, y);
        else if(x > mid) return queryAnd(rc, mid + 1, r, x, y);
        else return queryAnd(lc, l, mid, x, mid) & queryAnd(rc, mid + 1, r, mid + 1, y);
    }

    int queryOr(int rt, int l, int r, int x, int y) {
        if(l == x && y == r) return b[rt];
        int mid = l + r >> 1;
        push_down(rt);
        if(y <= mid) return queryOr(lc, l, mid, x, y);
        else if(x > mid) return queryOr(rc, mid + 1, r, x, y);
        else return queryOr(lc, l, mid, x, mid) | queryOr(rc, mid + 1, r, mid + 1, y);
    }
    ...
    int zero(int x) {
        ...
        while(lo <= hi) {
            int mid = lo + hi >> 1;
            if(queryAnd(1, 1, N - 1, x, mid)) lo = mid + 1;
    ...
    int one(int x) {
        ...
        while(lo <= hi) {
            int mid = lo + hi >> 1;
            if(queryOr(1, 1, N - 1, x, mid)) ans = mid, hi = mid - 1;
            else lo = mid + 1;
        }
        return ans;
    }
    ...
    int getAns() {
        ...
        while(lo <= hi) {
            int mid = lo + hi >> 1;
            if(queryOr(1, 1, N - 1, mid, N - 1)) ans = mid, lo = mid + 1;
            else hi = mid - 1;
        }
        return ans;
    }
    ...
    void add(int x) {
        int y = zero(x);
        if(y > x) update(1, 1, N - 1, x, y - 1, 0);
        update(1, 1, N - 1, y, y, 1);
    }

    void sub(int x) {
        int y = one(x);
        if(y > x) update(1, 1, N - 1, x, y - 1, 1);
        update(1, 1, N - 1, y, y, 0);
    }
...
inline void solve(){
    memset(st.lazy, -1, sizeof(st.lazy));
    read(n), read(q);
    for(int i = 1;i<=n;++i) {
        read(a[i]);
        st.add(a[i]);
