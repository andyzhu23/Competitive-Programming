...
const int N = 2e5 + 5;
int n, p[N], a[N], ans[N], cnt, up[N][33], mx, k;
set<int> e[N], bad;
bitset<N> ok;

inline void solve(){
    read(n);
    for(int i = 1;i<=n;++i) read(up[i][0]), ok[up[i][0]] = 1;
    iota(p + 1, p + n + 1, 1);
    for(int i = 1;i<=n;++i) {
        read(a[i]);
        ckmax(mx, a[i]);
    ...
    k = (mx - n) / cnt;
    for(int i = 31;~i;--i) if(k & (1 << i)) {
        for(int j = 1;j<=n;++j) p[j] = up[p[j]][i];
    }
    for(int i = 1;i<=n;++i) e[a[p[i]]].ins(i);
    for(int i = 1;i<=n;++i) if(!e[i].empty()) {
        ans[*e[i].begin()] = i;
        e[i].erase(e[i].begin());
        for(auto x : e[i]) bad.ins(x);
    } else {
        ans[*bad.begin()] = i;
        bad.erase(bad.begin());
    }
