...
inline void solve(){
    ...
    if(a[0] == a[1]) {
        ...
        while(a[k] == a[1]) ++k;
        --k;
        // k + 1 positions to place a[0]
        // all works except place a[0] at last
        // k ways in total
        // for all permutations, out of k + 1 ways to place a[0], a[1], k of them works
        cout<<fac(n) * k % MOD * fast_pow(k + 1, MOD - 2, MOD) % MOD<<'\n';
    } else cout<<0<<'\n';
}
