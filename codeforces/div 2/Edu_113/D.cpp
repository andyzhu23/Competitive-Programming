...
inline void solve(){
    ...
    for(int i = 0;i<n - 1;++i) {
        int l = x[i], r = x[i + 1];
        int a = lb(all(ver), mp(l, 0)) - ver.begin();
        int b = ub(all(ver), mp(r, INF)) - ver.begin();
        ll len = b - a;
        um<int, int> cnt;
        for(int i = a;i<b;i++) ++cnt[ver[i].sec];
        for(auto x : cnt) ans -= x.sec * (x.sec - 1) >> 1;
        ans += len * (len - 1) >> 1;
    }
    for(int i = 0;i<m - 1;++i) {
        int l = y[i], r = y[i + 1];
        int a = lb(all(hor), mp(l, 0)) - hor.begin();
        int b = ub(all(hor), mp(r, INF)) - hor.begin();
        ll len = b - a;
        um<int, int> cnt;
        for(int i = a;i<b;i++) ++cnt[hor[i].sec];
        for(auto x : cnt) ans -= x.sec * (x.sec - 1) >> 1;
        ans += len * (len - 1) >> 1;
    }
    cout<<ans<<'\n';
