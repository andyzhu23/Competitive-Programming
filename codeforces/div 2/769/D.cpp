...
const int N = 2e5 + 5;
int n, a[N], mn = 0, ans = 0;

struct segtree {
    int st[N << 2], lazy[N << 2];
    void push_down(int rt) {
        st[lc] = gcd(lazy[rt], st[lc]);
        st[rc] = gcd(lazy[rt], st[rc]);
        lazy[lc] = gcd(lazy[lc], lazy[rt]);
        lazy[rc] = gcd(lazy[rc], lazy[rt]);
        lazy[rt] = 0;
    }
...
inline void solve(){
    ...
    st.build(1, 1, n);
    for(int i = 1;i<=n;++i) {
        st.update(1, 1, n, 1, i, a[i]);
        int l = 1, r = i;
        bool flag = 0;
        while(l <= r) {
            int mid = l + r >> 1;
            int len = i - mid + 1;
            int x = st.query(1, 1, n, mid);
            if(len == x) {
                flag |= mid > mn;
                l = mid + 1;
