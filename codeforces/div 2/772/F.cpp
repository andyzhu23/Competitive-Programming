...
ll x[N], w[N], ans[N];
vector<int> v[N];
struct Q {int l, r, id;} query[N];

struct fenwick {
    ll c[N];
    fenwick() {
        memset(c, LLINF, sizeof(c));
    }
    void update(int a, ll b) {
        for(int i = a;i;i-=lowbit(i)) ckmin(c[i], b);
    }
    ll query(int a) {
        ll ans = LLINF;
        for(int i = a;i<N;i+=lowbit(i)) ckmin(ans, c[i]);
        return ans;
    }
};
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        ...
        if(!stk.empty()) v[i].pb(stk.back());
        stk.pb(i);
    }
    stk.clear();
    for(int i = n;i;--i) {
        while(!stk.empty() && w[stk.back()] > w[i]) stk.pop_back();
        if(!stk.empty()) v[stk.back()].push_back(i);
        stk.pb(i);
    }
    for(int i = 1;i<=q;++i) {
        int l = read(), r = read();
    ...
    sort(query + 1, query + q + 1, [&](Q& a, Q& b) {
        return a.r < b.r;
    });
    int r = 0;
    for(int i = 1;i<=q;++i) {
        while(r < query[i].r) {
            ++r;
            for(auto l : v[r]) fen.update(l, (x[r] - x[l]) * (w[r] + w[l]));
        }
        ans[query[i].id] = fen.query(query[i].l);
    }
