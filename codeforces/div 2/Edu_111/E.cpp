...
const int N = 2e5 + 5;
int n, k, dp[N], calc[20][N], cur[20][N];
char c[N];
...
bool ok(int x) {
    ...
    for(int j = 0;j<k;++j) {
        for(int i = n;i;--i) {
            calc[j][i] = calc[j][i + 1];
            cur[j][i] = c[i] == j + 'a' || c[i] == '?' ? cur[j][i + 1] + 1 : 0;
            if(cur[j][i] >= x) calc[j][i] = i + x - 1;
        }
    }
    dp[0] = 0;
    for(int i = 0;i<(1 << k);++i) {
        for(int j = 0;j<k;++j) if(i >> j & 1) {
            int k = i ^ (1 << j);
            if(dp[k] < inf && calc[j][dp[k] + 1]) dp[i] = min(dp[i], calc[j][dp[k] + 1]);
        }
    }
    return dp[(1 << k) - 1] <= n;
}
