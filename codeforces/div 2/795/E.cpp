...
inline void solve(){
    ...
    int tot = 0;
    for(auto x : dis) mp[x] = ++tot;
    for(int i = 1;i<=tot;++i) e[i].clear(), e2[i].clear();
    for(int i = 1;i<=n;++i) {
        l[i] = mp[l[i]];
        r[i] = mp[r[i]];
        e[l[i]].pb(i);
    ...
    for(int i = 1;i<=tot;++i) {
        ...
        if(u.size() && v.size()) {
            int vv = *u.begin();
            for(auto uu : u) f.merge(vv, uu);
            u.clear();
            u.ins(f.Find(vv));
            int aa = f.Find(vv);
            vv = *v.begin();
            for(auto uu : v) g.merge(uu, vv);
            v.clear();
            v.ins(g.Find(vv));
            int bb = g.Find(vv);
            stt.ins({aa, bb});
    ...
    for(auto[x, y] : stt) {
        st.ins({f.Find(x), g.Find(y)});
    }
    u.clear(), v.clear();
    for(int i = 1;i<=n;++i) {
        if(c[i]) u.ins(f.Find(i));
        else v.ins(g.Find(i));
    }
    ans += u.size() + v.size() - st.size();
    print(ans, '\n');
}
