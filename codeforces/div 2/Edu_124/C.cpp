...
inline void solve(){
    ...
    ll ans = min(abs(a[1] - b[1]) + abs(a[n] - b[n]), abs(a[n] - b[1]) + abs(a[1] - b[n]));
    ll mn1 = INF, mn2 = INF, mn3 = INF, mn4 = INF;
    for(int i = 1;i<=n;++i) {
        ckmin(mn1, abs(a[1] - b[i]));
        ckmin(mn2, abs(a[n] - b[i]));
        ckmin(mn3, abs(b[1] - a[i]));
        ckmin(mn4, abs(b[n] - a[i]));
    }
    ckmin(ans, mn1 + mn2 + mn3 + mn4);
    ckmin(ans, mn1 + mn4 + abs(b[1] - a[n]));
    ckmin(ans, mn2 + mn3 + abs(b[n] - a[1]));
    ckmin(ans, mn2 + mn4 + abs(b[1] - a[1]));
    ckmin(ans, mn1 + mn3 + abs(b[n] - a[n]));
    print(ans, '\n');
}
