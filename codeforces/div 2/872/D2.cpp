...
ll c(int u, int v) {
    if(v > u) return 0;
    return f[u] * fp(f[v], mod - 2, mod) % mod * fp(f[u - v], mod - 2, mod) % mod;
}
...
inline void solve(){
    ...
    dfs();
    for(int i = 1;i<=n;++i) {
        ll tmp = c(sz[i], k / 2) * c(n - sz[i], k / 2) % mod;
        ans = (ans + tmp) % mod;
    }
    print((ans * fp(c(n, k), mod - 2, mod) % mod + 1) % mod, '\n');
}
