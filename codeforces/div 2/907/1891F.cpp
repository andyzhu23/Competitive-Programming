...
inline void solve(){
    ...
    for(auto[t, v, x] : qq) {
        if(t == 1) {
            v = ++cnt;
            ll tmp = st.query(1, 1, n, dfn[v]);
            st.update(1, 1, n, dfn[v], dfn[v], -tmp);
            tmp = st.query(1, 1, n, dfn[v]);
        } else {
            st.update(1, 1, n, dfn[v], dfn[v] + sz[v] - 1, x);
        }
        // cout<<t<<' '<<v<<' '<<x<<' '<<st.query(1, 1, n, dfn[2])<<'\n';
    }
    for(int i = 1;i<=n;++i) print(st.query(1, 1, n, dfn[i]), " \n"[i==n]);
}
