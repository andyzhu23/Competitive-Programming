...
inline void solve(){
    ...
    int j = 0;
    int ans = INF;
    st.update(1, 1, m - 1, a[0].l, a[0].r, 1);
    if(st.ok()) {
        ans = min(ans, a[0].w - a[0].w);
    }
    for(int i = 0;i<a.size();i++) {
        j = max(i + 1, j);
        while(j < a.size() && !st.ok()) {
            st.update(1, 1, m - 1, a[j].l, a[j].r, 1);
            j++;
        }
        if(j > a.size()) break;
        if(st.ok()) ans = min(ans, a[j - 1].w - a[i].w);
        st.update(1, 1, m - 1, a[i].l, a[i].r, -1);
    }
    cout<<ans<<"\n";
}
