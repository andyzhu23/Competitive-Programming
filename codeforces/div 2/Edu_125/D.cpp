...
inline void solve(){
    int n = read(), c = read();
    for(int i = 1;i<=n;++i) {
        ll ci = read(), d = read(), h = read();
        v[i] = {ci, d * h};
    }
    sort(v + 1, v + n + 1);
    for(int i = 1;i<=n;++i) {
        auto[ci, dh] = v[i];
        if(ci == v[i + 1].fir) continue;
        ckmax(dp[ci], dh);
        ll tmp = dp[ci] << 1;
        for(ll j = ci << 1;j<=c;j += ci, tmp += dp[ci]) {
            ckmax(dp[j], tmp);
        }
    }
    for(int i = 1;i<N;++i) {
    ...
    for(int i = 1;i<=m;++i) {
        ll D = read(), H = readLL();
        if(dp[c] <= D * H) print(-1, i == m ? '\n' : ' ');
        else print(ub(dp, dp + c + 1, D * H) - dp, i == m ? '\n' : ' ');
    }
}
