...
inline void solve() {
    ...
    for(int i = 1;i<=n;++i) {
        ...
        while(i > p[i] + 1 && p[i] + 1 + i <= n && c[i - (p[i] + 1)] == c[i + p[i] + 1]) ++p[i];
        if(i + p[i] > r) mid = i, r = i + p[i];
        if(c[i] == '#') st.update(1, 1, n, i, i + p[i], i);
    }
    ll ans = 0;
    for(int i = 1;i<=n;++i) if(c[i] != '#') {
        int x = st.query(1, 1, n, i);
        if(2 * x - i - 2 >= 0) dp[i] = dp[2 * x - i - 2] + 1;
        ans += dp[i];
    }
    print(ans, '\n');
}
