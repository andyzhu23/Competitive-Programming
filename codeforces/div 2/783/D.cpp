...
struct fenwick {
    ...
    void update(int a, ll b) {
        for(int i = a;i<=len;i+=i&(-i)) ckmax(c[i], b);
    }
    ll query(int a) {
        ll ans = -LLINF;
        for(int i = a;i;i-=i&(-i)) ckmax(ans, c[i]);
        return ans;
    }
    void clear() {
        for(int i = 0;i<=len;++i) c[i] = -LLINF;
    }
} fen;

inline void solve(){
    n = read();
    vll a(n + 5), psa(n + 5), dp(n + 5);
    set<ll> dis;
    dis.ins(0);
    len = 0;
    for(int i = 1;i<=n;++i) {
    ...
    for(int i = 1;i<=n;++i) {
        ...
        ckmax(dp[i], fen.query(mp[psa[i]] - 1) + i);
        fen.update(mp[psa[i]], dp[i] - i);
    }
    print(dp[n], '\n');
}
