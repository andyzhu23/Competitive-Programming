...
struct node {
    int len;
    bool type;
    // length of the sequence, the first type: 1 is square, 0 is round
    // Since the sequence you end up will always just be ([([([([
    node operator+(const node& other) const {
        bool lstA = type;
        if((len & 1) == 0) lstA ^= 1;
        if(lstA != other.type) return {len + other.len, type};
        else {
            if(len > other.len) return {len - other.len, type};
            else return {other.len - len, len & 1 ? !other.type : other.type};
        }
    }
};
...
inline void solve(){
    ...
    segtree st(n);
    for(int i = 1;i<=n;++i) {
        if(s[i] == '(' || s[i] == ')') st.update(1, 1, n, i, {1, 0});
        else st.update(1, 1, n, i, {1, 1});
    }
    int q; cin>>q;
    while(q--) {
        int l, r; cin>>l>>r;
