...
inline void solve(){
    ...
    mp.clear();
    vis.reset();
    for(int i = 1;i<=n;++i) read(a[i]), mp[a[i]] = i;
    int l, r;
    l = r = (*mp.begin()).sec;
    int mex = 0;
    ll ans = 0;
    while(!mp.empty()) {
        ...
        mp.erase(mp.begin());
        vis[_] = 1;
        while(i < l) mp.erase(a[--l]), vis[a[l]] = 1;
        while(i > r) mp.erase(a[++r]), vis[a[r]] = 1;
        while(vis[mex]) ++mex;
        auto[__, j] = *mp.begin();
        int x, y;
        int ex = mex * 2 - (r - l + 1);
        if(j > r) {
            x = j - r - 1, y = l - 1;
            for(int i = r;i<j;++i) ans += max(0, min(y + 1, ex - (i - r) + 1));
        }
        else {
            x = n - r, y = l - j - 1;
            for(int i = l;i>j;--i) ans += max(0, min(x + 1, ex - (l - i) + 1));
        }
    }
    print(ans, '\n');
}
