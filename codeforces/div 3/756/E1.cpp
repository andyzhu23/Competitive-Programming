...
inline void solve() {
    read(n), read(k);
    for(int i = 1;i<=n;++i) e[i].clear(), dist[i] = 0, c[i] = INF;
    ...
    c[0] = dist[0] = -1;
    dfs(1);
    for(int i = 1;i<=k;++i) q.push({x[i], 0});
    while(!q.empty()) {
        pii cur = q.front(); q.pop();
        int u = cur.first, fa = cur.second;
        if(c[u] != INF) continue;
        c[u] = c[fa] + 1;
        for(int v : e[u]) q.push({v, u});
    }
    for(int i = 2;i<=n;++i) {
        if(e[i].size() == 1) {
            if(c[i] > dist[i]) {
