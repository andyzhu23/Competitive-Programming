...
inline void solve(){
    n = read(), k = read();
    a.clear();
    for(int i = 1;i<=k;++i) a.insert(read());
    for(int i = 1;i<=n;++i) dist[i] = 0, e[i].clear(), dp[i] = 0, ok[i] = 0;
    dist[0] = -1;
    for(int i = 1;i<n;++i) {
        int u = read(), v = read();
        e[u].pb(v);
