    ...
    for(int i = 0;i<s.size();i++) {
        ...
        if(k > vis.size()) {
            ...
            if(*used.rbegin() >= cur) {
                ...
                while(k == vis.size() && (ans % 10 == *used.rbegin() && used.count(ans % 10) > 1)) {
                    used.erase(used.find(ans % 10));
                    if(!used.count(ans % 10)) 
                        vis.erase(ans % 10);
                    ans /= 10;
                    --i;
                }
                if(k > vis.size() || used.count(ans % 10) == 1) {
                    used.erase(used.find(ans % 10));
                    if(!used.count(ans % 10)) 
                        vis.erase(ans % 10);
                    ans ++;
                    ...
                    if(!used.count(ans % 10)) 
                        vis.erase(ans % 10);
                    ans = ans - ans % 10 + *used.ub(ans % 10);
                }
                int last = vis.size() < k ? 0 : *used.begin();
