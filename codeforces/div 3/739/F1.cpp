...
bool isa(ll ans) {
    ...
    ans /= 10;
    while(ans > 0) {
        if(prev != ans % 10) return 0;
        ans /= 10;
    }
    return 1;
}

void dfs(int x, int y, ll ans, int cnt) {
    if(cnt == 12) return;
    if(isa(ans)) {
        a.ins(ans);
    }
    b.ins(ans);
    dfs(x, y, ans * 10 + x, cnt + 1);

    dfs(x, y, ans * 10 + y, cnt + 1);
}
...
inline void solve(){
    ...
    ll ans = LLINF;
    int pos;
    if(*a.lb(n) == n) ans = min(ans, *a.lb(n));
    else ans = min(ans, *a.ub(n));
    if(k == 1) {
        cout<<ans<<'\n';
        return;
    }
    if(*b.lb(n) == n) ans = min(ans, *b.lb(n));
    else ans = min(ans, *b.ub(n));
    cout<<ans<<'\n';
}
