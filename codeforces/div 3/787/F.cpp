...
vi e[N], path;
bool flag;
bitset<N> in, vis, mark;

void get(int u = x, int fa = 0) {
    path.pb(u);
    in[u] = 1;
    if(u == y) {
        flag = 1;
...
void dfs(int u, int fa = 0) {
    if(vis[u]) mark[u] = 1;
    for(int v : e[u]) if(v != fa && !in[v]) {
        dfs(v, u);
        mark[u] = mark[u] | mark[v];
    }
    if(mark[u] && !in[u]) ans += 2;
}
...
inline void solve(){
    ...
    path.clear();
    flag = ans = 0;
    for(int i = 1;i<=n;++i) e[i].clear(), in[i] = vis[i] = mark[i] = 0;
