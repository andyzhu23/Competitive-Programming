...
int dp[N], last[N];
char c[N][N], s[N];
unordered_map<int, pii> vis1, vis2;

void dfs(int u) {
    if(u == 0) return;
    dfs(last[u]);
    int res = 0;
    for(int i = last[u] + 1;i<=u;++i) {
        res = res * 131 + s[i];
    }
    if(vis1[res] != (pii){0, 0}) {
        print(vis1[res].sec, ' ');
        print(vis1[res].sec + 1, '\n');
        print(vis1[res].fir, ' ');
    } else if(vis2[res] != (pii){0, 0}) {
        print(vis2[res].sec, ' ');
        print(vis2[res].sec + 2, '\n');
        print(vis2[res].fir, ' ');
    }
}
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        scanf("%s", c[i] + 1);
        for(int j = 1;j<m;++j) {
            vis1[c[i][j] * 131 + c[i][j + 1]] = {i, j};
        }
        for(int j = 1;j<m - 1;++j) {
            vis2[c[i][j] * 131 * 131 + c[i][j + 1] * 131 + c[i][j + 2]] = {i, j};
        }
    }
    scanf("%s", s + 1);
    dp[0] = 1;
    for(int i = 1;i<=m;++i) {
        // cout<<(char)s[i - 1]<<' '<<(char)s[i]<<'\n';
        if(i >= 2 && vis1[s[i - 1] * 131 + s[i]] != (pii){0, 0}) {
            // cout<<i<<' '<<i - 2<<'\n';
            dp[i] |= dp[i - 2];
            if(dp[i - 2]) last[i] = i - 2;
        }
        if(i >= 3 && vis2[s[i - 2] * 131 * 131 + s[i - 1] * 131 + s[i]] != (pii){0, 0}) {
            dp[i] |= dp[i - 3];
            if(dp[i - 3]) last[i] = i - 3;
        }
    }
    if(!dp[m]) {
        puts("-1");
