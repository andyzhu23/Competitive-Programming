...
int dp[2][2005]; // minimum size ending up at a position relative to the left endpoint
...
inline void solve() {
    ...
    for(int i = 1;i<=n;++i) {
        ...
        for(int j = 0;j<=2000;++j) {
            // go left
            int sz = dp[i & 1 ^ 1][j] + max(0, x - j);
            int pos = max(0, j - x);
            if(sz <= 2000 && pos <= 2000) 
                dp[i & 1][pos] = min(dp[i & 1][pos], sz);
            // go right
            sz = max(dp[i & 1 ^ 1][j], x + j);
            pos = x + j;
            if(sz <= 2000 && pos <= 2000) 
                dp[i & 1][pos] = min(dp[i & 1][pos], sz);
        }
    ...
    int ans = INF;
    for(int i = 0;i<=2000;++i) ans = min(dp[n & 1][i], ans);
    cout<<ans<<'\n';
}
