...
inline void solve(){
    ...
    ll mn = LLINF, mx = 0, a = 0, b = 0, d = 0;
    c[0] = 0x3f3f3f3f3f3f3f3f;
    l[0] = LLINF, r[0] = 0;
    for(int i = 1;i<=n;++i) {
        l[i] = read(), r[i] = read(), c[i] = read();
        if(l[i] < mn || (l[i] == mn && c[i] < c[a])) mn = l[i], a = i;
        if(r[i] > mx || (r[i] == mx && c[i] < c[b])) mx = r[i], b = i;
        if(l[d] > mn || r[d] < mx) d = 0;
        if(l[i] == mn && r[i] == mx && c[i] < c[d]) d = i;
        if(d == 0) print(c[a] + c[b], '\n');
        else if(c[d] < c[a] + c[b]) print(c[d], '\n');
        else print(c[a] + c[b], '\n');
    }
}
