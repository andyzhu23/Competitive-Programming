...
inline void dfs(int u = 1, int fa = 0) {
    ...
    for(int v : e[u]) {
        ...
        dfs(v, u);
        sz[u] += sz[v];
        for(int i = 0;i<32;++i) if(dp[u][cnt - 1] >> i & 1) {
            for(int j = 0;j<32;++j) if(dp[v].back() >> j & 1) {
                dp[u][cnt] |= 1 << (i ^ j);
            }
        }
    }
    if(sz[u] & 1 ^ 1) dp[u].back() |= 1;
}

vi ans;

inline void dfs2(int u = 1, int fa = 0, int val = 0) {
    if(val == 0 && (sz[u] & 1 ^ 1)) ans.pb(u);
    int cnt = e[u].size();
    reverse(all(e[u]));
    for(int v : e[u]) {
        for(int i = 0;i<32;++i) if(dp[u][cnt - 1] >> i & 1) {
            if(dp[v].back() >> (val ^ i) & 1) {
                dfs2(v, u, val ^ i);
                val = i;
                break;
            }
