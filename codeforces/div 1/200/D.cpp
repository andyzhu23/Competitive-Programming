...
int n, q;
vi e[N];
int hson[N], f[N], dfn[N], top[N], sz[N], tot;
...
void dfs2(int u = 1, int fa = 0, int tp = 1) {
    top[u] = tp;
    dfn[u] = ++tot;
    for(int v : e[u]) if(v != fa && sz[v] > sz[hson[u]]) hson[u] = v;
    if(hson[u]) dfs2(hson[u], u, tp);
    for(int v : e[u]) if(v != fa && v != hson[u]) {
        dfs2(v, u, v);
    }
}
...
void update(int v, int val) {
    while(v) {
        st.update(1, 1, n, dfn[top[v]], dfn[v], val);
        v = f[top[v]];
    }
}
...
inline void solve(){
    ...
    while(q--) {
        int opt = read(), v = read();
        if(opt == 1) st.update(1, 1, n, dfn[v], dfn[v] + sz[v] - 1, 1);
        else if(opt == 2) update(v, 0);
        else print(st.query(1, 1, n, dfn[v]), '\n');
    }
}
