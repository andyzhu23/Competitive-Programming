...
void get(int u, int fa = 0) {
    for(auto[v, w] : e[u]) if(v != fa && v != hson[u]) {
        tmp.clear();
        get(v, u);
    }
    tmp.clear();
    if(hson[u]) get(hson[u], u);
    tmp.ins(d[u]);
    for(auto[v, w] : e[u]) if(v != fa && v != hson[u]) {
        collect(v, u);
    }
    for(int i = 1;i<=q;++i) {
        ans[i] = max(ans[i], (int)tmp.order_of_key(d[u] + l[i] + 1));
    }
}
