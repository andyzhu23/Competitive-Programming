...
struct Dinic {
    int S, T, tot = 1, head[N], dist[N], cur[N];
    struct edge {int u, v, w, f, nxt;} e[100005];
...
inline void solve(){
    ...
    dinic.S = 0;
    for(int i = 1;i<=n;++i) {
        for(int j = i + 1;j<=n;++j) if(!vis.count({i, j}) && !vis.count({j, i})) {
            ++tot;
            dinic.add(dinic.S, tot, 1, 0);
            dinic.add(tot, i, 1, 0);
            mp[(pii){i, j}] = dinic.tot - 1;
            dinic.add(tot, j, 1, 0);
        }
    }
    dinic.T = tot + 1;
    for(int i = 1;i<=n;++i) {
        for(int j = mn[i];j<=mx[i];++j) {
            dinic.add(i, dinic.T, 1, 2 * j - 1);
        }
    }
    dinic.dinic();
    for(auto[u, v] : mp) {
        if(dinic.e[v].f) ans[u.sec][u.fir] = 1;
        else ans[u.fir][u.sec] = 1;
    }
    for(int i = 1;i<=n;++i) {
