...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        read(a[i]);
        if(!mp.count(a[i])) mp[a[i]] = (pii){cnt[a[i]] * 2 - i + 1, i};
        ckmin(mp[a[i]], (pii){cnt[a[i]] * 2 - i + 1, i});
        ++cnt[a[i]];
        if(ans < cnt[a[i]] * 2 - i - mp[a[i]].fir) {
            ans = cnt[a[i]] * 2 - i - mp[a[i]].fir;
            x = a[i];
            y = mp[a[i]].sec;
            z = i;
        }
