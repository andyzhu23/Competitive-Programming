...
struct DSU {
    int n, f[N], siz[N], g[N], siz2[N];
    vector<pii> heap2;
    unordered_set<int> vis2;
    DSU(int n) :n(n) {
        for(int i = 0;i<=n;++i) f[i] = i, siz[i] = 0, heap.push({0, i});
    }
    ...
    void copy() {
        for(int i = 0;i<=n;++i) g[i] = f[i], siz2[i] = siz[i];
        heap2.clear(), vis2.clear();
        while(!heap.empty()) heap2.pb(heap.top()), heap.pop();
        for(auto x : heap2) heap.push(x);
        for(auto x : vis) vis2.insert(x);
    }
    void paste() {
        for(int i = 0;i<=n;++i) f[i] = g[i], siz[i] = siz2[i];
        while(!heap.empty()) heap.pop();
        vis.clear();
        for(auto x : heap2) heap.push(x);
        for(auto x : vis2) vis.insert(x);
    }
};
...
inline void solve(){
    ...
    while(d--) {
        ...
        heap.push({dsu.siz[dsu.Find(a)], dsu.Find(a)});
        ans2 = ans;
        dsu.copy();
        for(int i = 1;i<=cnt;++i) {
            while(!heap.empty() && vis.count(heap.top().sec)) heap.pop();
            pii u = heap.top(); heap.pop();
            while(!heap.empty() && (vis.count(heap.top().sec) || heap.top().sec == u.sec)) heap.pop();
            pii v = heap.top(); heap.pop();
            dsu.merge(u.sec, v.sec);
            heap.push({dsu.siz[dsu.Find(u.sec)], dsu.Find(u.sec)});
            ans = max(ans, dsu.siz[dsu.Find(u.sec)]);
        }
        ans = max(ans, dsu.siz[dsu.Find(a)]);
        print(ans, '\n');
        dsu.paste();
        ans = ans2;
        ans = max(ans, dsu.siz[dsu.Find(a)]);
