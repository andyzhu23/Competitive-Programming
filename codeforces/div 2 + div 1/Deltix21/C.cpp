...
const int N = 1e3 + 10;
ll a[N];
// cnt1: total left brackets not paired
// cnt2: total right brackets not paired
ll cnt1, cnt2;
stack<ll> stk;
// if paired brackets have the same number of left brackets
// not cleared, then they can be parallel
um<ll, ll> cnt3;
...
inline void solve(){
    ...
    for(int i = 1;i<=n;i++) {
        if(i & 1) {
            ...
            cnt2 = a[i];
            int pre = ans;
            while(!stk.empty() && cnt2 > 0) {
                ll cur = stk.top(); stk.pop();
                if(cur - cnt2 >= 0) {
                    cur -= cnt2;
                    if(cur > 0) stk.push(cur);
                    cnt1 -= cnt2;
                    // those higher than cnt1 cannot be parallel anymore
                    for(auto& x : cnt3)
                        if(x.fir > cnt1) x.sec = 0;
                    ans += cnt2 + cnt3[cnt1];
                    cnt2 = 0;
                    cnt3[cnt1] ++;
                    break;
            ...
            // clear when right brackets cannot all be paired
            if(cnt2 > 0) cnt3.clear();
        }
    }
    cout<<ans<<'\n';
}
