...
const int N = 1e5 + 10;
int n, q;
ll a[N], b[N], sum[N], Min[N][22], Max[N][22];
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        Min[i][0] = Max[i][0] = sum[i] = sum[i - 1] + (b[i] - a[i]);
    }
    for(int j = 1;j<22;++j) {
    ...
    while(q--) {
        int l, r;
        cin>>l>>r;
        if(query2(l, r) - sum[l - 1] < 0 || sum[r] - sum[l - 1] != 0) cout<<-1<<'\n';
        else cout<<query1(l, r) - sum[l - 1]<<'\n';
    }
}
