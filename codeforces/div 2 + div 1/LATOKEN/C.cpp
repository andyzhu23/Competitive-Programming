...
template <typename T1, typename T2> inline T1 fast_pow(T1 a, T2 b){if(b == 1) return a % MOD; T1 tmp = fast_pow(a, b / 2); if(b % 2 == 1) return tmp * tmp % MOD * a % MOD; else return tmp * tmp % MOD;}
...
void dfs(int u, int& cnt, vi& posa, vi& posb, vi& vis, vi& a, vi& b) {
    if(vis[u]) return;
    vis[u] = cnt;
    dfs(posa[b[u]], cnt, posa, posb, vis, a, b);
}
...
inline void solve(){
    ...
    for(int i = 1;i<=n;i++){
        if(vis[i]) continue;
        dfs(i, ++cnt, posa, posb, vis, a, b);
    }
    cout<<fast_pow(ll(2), cnt)<<endl;
}
