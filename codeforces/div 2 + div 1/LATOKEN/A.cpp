...
inline void init1(){
    ...
    for(int i = 0;i<N;i++){
        for(int j = 0;j<N;j++){
            if(i == 0 && j == 0) continue;
            if(j == 0) {
                opt1[i][j] = opt1[i - 1][j] == 'R' ? 'W' : 'R';
                opt2[i][j] = opt2[i - 1][j] == 'R' ? 'W' : 'R';
                continue;
            }
            opt1[i][j] = opt1[i][j - 1] == 'R' ? 'W' : 'R';
            opt2[i][j] = opt2[i][j - 1] == 'R' ? 'W' : 'R';
        }
    }
}
...
inline void solve(){
    ...
    for(int i = 0;i<n;i++){
        for(int j = 0;j<m;j++){
            if(a[i][j] == '.') continue;
            if(a[i][j] != opt1[i][j]) flag1 = false;
            if(a[i][j] != opt2[i][j]) flag2 = false;
        }
    }
    if(!flag1 && !flag2){
        cout<<"NO"<<endl;
