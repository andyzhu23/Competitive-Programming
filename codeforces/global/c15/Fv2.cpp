...
struct fenwick {
    ...
    ll query(int a) {
        ll ans = 0;
        for(int i = a;i;i-=i & (-i)) ans = (c[i] + ans) % MOD;
        return ans;
    }
    ll query(int a, int b) {
        return (query(b) + MOD - query(a - 1)) % MOD;
    }
    void update(int a, ll b) {
        b -= query(a, a);
        for(int i = a;i<=n;i+=i & (-i)) c[i] = (c[i] + b) % MOD;
    }
...
inline void solve(){
    ...
    ll ans = 0;
    for(int i = 1;i<=n;++i) {
        fen.update(i, x[i] - y[i]);
        fen.update(i, fen.query(Map[*dis.ub(y[i])], Map[x[i]]));
    }
    for(int i = 1;i<=n;++i) {
        ans += x[i] - x[i - 1];
        ans %= MOD;
        if(!s[i]) continue;
        ans += fen.query(Map[x[i]], Map[x[i]]);
        ans %= MOD;
    }
    ++ans;
    ans %= MOD;
