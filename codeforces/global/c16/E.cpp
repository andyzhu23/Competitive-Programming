...
void dfs(int u, int fa, vi e[], vi& cnt, vi& cut) {
    if(e[u].size() == 1 && u != 1) {
        // is leaf
        cnt[u]++;
        return;
    ...
    if((e[u].size() - cut[u] == 1 && u != 1) || (u == 1 && e[u].size() - cut[u] == 0)) {
        // check is leaf again after cutting
        if(cnt[u] > 0) return;
        cnt[u]++;
        return;
    }
