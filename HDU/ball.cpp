...
namespace comfun {
    ...
    template <typename T> inline bool is_prime(T x){if(x == 1 || x == 0) return false; for(T i = 2; i * i <= x;i++) if(x % i == 0) return false; return true;}
}
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        for(int j = i + 1;j<=n;++j) {
            e.pb({i, j, abs(a[i].fir - a[j].fir) + abs(a[i].sec - a[j].sec)});
        }
    }
    sort(all(e));
    int ans = 0;
