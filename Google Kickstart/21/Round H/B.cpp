...
inline void solve(){
    ...
    string s; cin>>s;
    s = '@' + s;
    vector<int> red(n + 1), blue(n + 1), yellow(n + 1);
    for(int i = 1;i<=n;++i) {
        red[i] = red[i - 1];
        if(s[i] == 'R' || s[i] == 'O' || s[i] == 'P' || s[i] == 'A') {
            red[i] = red[i - 1] + 1;
            if(s[i - 1] == 'R' || s[i - 1] == 'O' || s[i - 1] == 'P' || s[i - 1] == 'A'){
                red[i]--;
            }
        }
        blue[i] = blue[i - 1];
        if(s[i] == 'B' || s[i] == 'P' || s[i] == 'G' || s[i] == 'A') {
            blue[i] = blue[i - 1] + 1;
            if(s[i - 1] == 'B' || s[i - 1] == 'P' || s[i - 1] == 'G' || s[i - 1] == 'A'){
                blue[i]--;
            }
        }
        yellow[i] = yellow[i - 1];
        if(s[i] == 'Y' || s[i] == 'O' || s[i] == 'G' || s[i] == 'A') {
            yellow[i] = yellow[i - 1] + 1;
            if(s[i - 1] == 'Y' || s[i - 1] == 'O' || s[i - 1] == 'G' || s[i - 1] == 'A'){
                yellow[i]--;
            }
        }
    }
    print(red[n] + yellow[n] + blue[n], '\n');
}
