...
void getg(int u) {
    ...
    for(int v : g[u]) {
        getg(v);
        depg[u] = max(depg[u], depg[v] + 1);
        szg[u] += szg[v];
    }
}

bool dfs(int u, int v) {
    if(depe[u] != depg[v] || sze[u] != szg[v]) return 0;
    if(u < 0 && v < 0) return u == v;
    if(1ll * u * v < 0) return 0;
    int uu = e[u][0];
    int vv = g[v][0];
    int uuu = e[u][1];
    int vvv = g[v][1];
    return (dfs(uu, vv) && dfs(uuu, vvv)) || (dfs(uu, vvv) && dfs(uuu, vv));
}
...
inline void solve(){
    ...
    gete(1);
    getg(1);
    puts(dfs(1, 1) ? "Fred and Mary might have the same mobile." : "Fred and Mary have different mobiles.");
}
