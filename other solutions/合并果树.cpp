...
inline void solve(){
    read(n);
    for(int i = 1;i<=n;++i) read(a[i]), dis.ins(a[i]);
    for(auto x : dis) mp[x] = ++tot, id[tot] = x;
    ...
    while(q--) {
        ...
        cl = N, dl = N, cr = N - 1, dr = N - 1;
        for(int i = 1;i<=n;++i) for(int j = 1;j<=cnt[i];++j) c[++cr] = id[i];
        while(cr + dr - cl - dl + 2 > 1) {
            d[++dr] = get() + get();
            ans += d[dr];
        }
        print(ans, '\n');
