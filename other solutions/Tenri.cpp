...
inline void solve(){
    ...
    for(int i = 0;i<n;++i) {
        for(int j = 0;j<=s;++j) {
            dp[1 << i][j] = (a[i] + j) * (a[i] + j) + j;
        }
    }
    for(int i = 0;i<(1 << n);++i) { // state of all boxes
        int cnt = 0;
        for(int j = 0;j<n;++j) cnt += 1 & i >> j;
        if(cnt & 1) continue;
        for(int x = 0;x<n;++x) if(!(1 & i >> x)) { // most outside box
            for(int u = 1;u<i;++u) if((u | i) == i){ // state of the left box
                int v = i - u; // state of the right box
                int cnt = 0;
                for(int j = 0;j<n;++j) cnt += 1 & u >> j;
                if(cnt % 2 == 0) continue;
                for(int j = 0;j<=s;++j) { // number of sparklers
                    for(int k = 0;k<=j;++k) { // number of sparklers for the left box
                        for(int l = 0;l + k<=j;++l) { // number of sparklers for the right box
                            ckmax(dp[i | (1 << x)][j], min(dp[u][k] * dp[v][l], (a[x] + j - k - l) * (a[x] + j - k - l)) + j - k - l);
                        }
                    }
                }
            }
