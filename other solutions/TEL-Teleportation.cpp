...
inline void solve(){
    ...
    for(int v : e[1]) l2.ins(v), ll2.ins(v);
    for(int v : e[2]) l4.ins(v), ll4.ins(v);
    for(int i = 3;i<=n;++i) if(!l2.count(i) && !l4.count(i)) {
        l3.ins(i);
        ll3.ins(i);
    }
    ll ans = 0;
    ...
    for(auto u : l3) {
        ...
        ll3.erase(u);
        for(int v : e[u]) {
            if(l2.count(v)) ++cnt, flag = 2;
            if(ll3.count(v)) ++cnt2;
            if(l4.count(v)) ++cnt, flag = 4;
        }
        ans += ll3.size() - cnt2;
        if(flag == 0) ans += max(l2.size(), l4.size()) - cnt;
        if(flag == 2) ans += l2.size() - cnt;
        if(flag == 4) ans += l4.size() - cnt;
    }
    cout<<ans<<'\n';
}
