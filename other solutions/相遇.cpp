...
bool ok(int a, int b, int c, int d, int flag = 0) {
    int lca = LCA(a, b);
    if(LCA(lca, c) == lca ^ LCA(lca, d) == lca) return 1;
    if(LCA(c, d) == lca) return 1;
    if(flag) return 0;
    return ok(c, d, a, b, 1);
}
