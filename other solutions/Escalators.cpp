...
ll get(int x) {
    DSU dsu(n);
    vis.reset();
    for(auto[u, v] : e) if((a[u] >> x & 1) & (a[v] >> x & 1)) {
        dsu.merge(u, v);
    }
    ll ret = 0;
    for(int i = 0;i<n;++i) if((a[i] >> x & 1) && !vis[dsu.Find(i)]) {
        vis[dsu.Find(i)] = 1;
        ret += 1ll * dsu.sz[dsu.Find(i)] * (dsu.sz[dsu.Find(i)] - 1) >> 1 << x;
    }
    return ret;
}
...
inline void solve(){
    read(n);
    ll ans = 0;
    for(int i = 0;i<n;++i) read(a[i]), ans += a[i];
    for(int i = 1;i<n;++i) {
        int u = read(), v = read();
        e.pb({u, v});
    }
    for(int i = 0;i<20;++i) ans += get(i);
    print(ans, '\n');
}
