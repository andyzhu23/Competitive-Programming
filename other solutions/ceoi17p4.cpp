...
struct lichaotree {
    ...
    ll calc(int idx, int pos) {
        return c[idx].fir + c[idx].sec * pos;
    }
    void update(int rt, int l, int r, int x, int y, int u) {
        ...
        if(x <= l || y >= r) {
            int v = st[rt];
            if(c[u].sec > c[v].sec) swap(u, v);
            ll resu = calc(u, mid), resv = calc(v, mid);
            if(c[u].sec == c[v].sec) st[rt] = resu > resv ? v : u;
            else if(resu > resv) {
                st[rt] = v;
    ...
    void update(ll a, ll b) {
        add(a, b);
        update(1, 0, H-5, 0, H-5, tot);
    }
    ll query(int rt, int l, int r, int x) {
        if(l > x || r < x) return llinf;
        if(l == r) return calc(st[rt], x);
        int mid = l + r >> 1;
        return min({calc(st[rt], x), query(lc, l, mid, x), query(rc, mid + 1, r, x)});
    }
} st;
...
inline void solve() {
    ...
    for(int i = 1;i<=n;++i) psa[i] = psa[i - 1] + read();
    dp[1] = 0;
    st.update(dp[1] - psa[1] + h[1] * h[1], -2 * h[1]);
    for(int i = 2;i<=n;++i) {
        dp[i] = h[i] * h[i] + psa[i - 1] + st.query(1, 0, H-5, h[i]);
        st.update(dp[i] - psa[i] + h[i] * h[i], -2 * h[i]);
    }
    print(dp[n], '\n');
}
