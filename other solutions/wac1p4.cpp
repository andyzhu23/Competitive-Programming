...
void dfs(int u = 1, int fa = 0) {
    dp[u][1][s[u] == 'R'][s[u] == 'B'] = 1;
    for(int v : e[u]) if(v != fa) {
        dfs(v, u);
        for(int i = sz[u];i;--i) {
            for(int j = 1;j<=min(i - 1, sz[v]);++j) {
                for(int ii = 0;ii<=2;++ii) {
                    for(int jj = 0;jj<=2;++jj) {
                        for(int iii = 0;iii<=2;++iii) {
                            for(int jjj = 0;jjj<=2;++jjj) {
                                dp[u][i][min(2, ii + iii)][min(2, jj + jjj)] = 
                                    (dp[u][i][min(2, ii + iii)][min(2, jj + jjj)] + dp[u][i - j][ii][jj] * dp[v][j][iii][jjj] % mod) % mod;
                            }
                        }
                    }
                }
