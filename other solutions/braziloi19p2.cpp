...
int dfn[N], low[N], tot, sz[N], color, c[N];
bitset<N> vis;
vi stk;
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) if(!vis[c[i]]) {
        ...
        if(sz[c[i]] == 1) {
            sort(all(e[i]));
            for(int j = 0;j<(int)e[i].size()-1;++j) ans += e[i][j].fir;
        } else {
            int u = i;
            vi opt;
            do {
                int mx = 0;
                for(auto[w, v] : e[to[u]]) if(v != u) {
                    ckmax(mx, w);
                    ans += w;
                }
                opt.pb(f[u] - mx);
            ...
            sort(all(opt));
            ans += opt[0];
            for(int i = 1;i<opt.size();++i) if(opt[i] < 0) ans += opt[i];
        }
    }
    print(ans, '\n');
}
