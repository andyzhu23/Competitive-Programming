...
inline void solve(){
    ...
    for(int i = 1;i<=m;++i) {
        int s = read(), t = read(), a = read(), b = read(), c = read(), d = read();
        e.pb({s, t, a, b, c, d, i});
    }
    sort(all(e));
    ...
    if(p == 1) ckmin(ans, t);
    for(auto x : e) {
        while(!tmp.empty() && tmp.top().d <= x.a) {
            ckmin(mn[tmp.top().t], dp[tmp.top().id] - tmp.top().c);
            tmp.pop();
        }
        ckmin(dp[x.id], mn[x.s] + x.b);
        if(x.t == p && x.d <= t) ckmin(ans, dp[x.id] + t - x.c);
        tmp.push(x);
    }
