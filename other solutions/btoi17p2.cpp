...
struct matrices {
    int a[5][5];
    matrices operator*(const matrices& o) const {
        matrices ans;
        memset(ans.a, 0x3f, sizeof ans.a);
        for(int k = 0;k<5;++k) {
            for(int i = 0;i<5;++i) {
                for(int j = 0;j<5;++j) {
                    ckmin(ans.a[i][j], a[i][k] + o.a[k][j]);
                }
            }
        }
        return ans;
...
struct segtree {
    ...
    matrices query(int rt, int l, int r, int x, int y) {
        if(x > y) {
            matrices tmp;
...
inline void solve(){
    read(k), read(n), read(m), read(o);
    for(auto& x : a) memset(x.a, 0x3f, sizeof x.a);
    while(m--) {
        int u = read(), v = read(), w = read();
        ckmin(a[u / k].a[u % k][v % k], w);
    }
    st.build(1, 0, n);
    while(o--) {
        int l = read(), r = read();
        ...
        matrices ans = st.query(1, 0, n, l / k, r / k - 1);
        if(ans.a[l % k][r % k] == inf) print(-1, '\n');
        else print(ans.a[l % k][r % k], '\n');
    }
}
