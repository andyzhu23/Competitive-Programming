...
inline void solve(){
    ...
    for(int i = 1;i<m;++i) if(a[i].x == a[i - 1].x) {
        e[a[i].id].pb({a[i - 1].id, 2 * (a[i].y - a[i - 1].y)});
        e[a[i - 1].id].pb({a[i].id, 2 * (a[i].y - a[i - 1].y)});
    }
    sort(all(a), [&](node& a, node& b) {
        return a.y == b.y ? a.x < b.x : a.y < b.y;
    });
    for(int i = 1;i<m;++i) if(a[i].y == a[i - 1].y) {
        e[m + a[i].id].pb({m + a[i - 1].id, 2 * (a[i].x - a[i - 1].x)});
        e[m + a[i - 1].id].pb({m + a[i].id, 2 * (a[i].x - a[i - 1].x)});
    }
