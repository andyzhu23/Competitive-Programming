...
inline void solve(){
    ...
    for(int i = 1;i<=Q;++i) {
        int a = read(), b = read();
        q.pb({min(l[a], l[b]), max(r[a], r[b]), i});
    }
    sort(all(q));
    ...
    for(int i = 0;i<q.size();++i) {
        while(j == 0 || l[j + 1] < q[i].l) {
            ++j;
            st.update(1, 1, tot, mp[r[j]], (node){v[j], r[j] - l[j] + 1, j});
        }
        ans[q[i].id] = st.query(1, 1, tot, mp[q[i].r] + 1, tot).id;
    }
