...
inline void solve(){
    read(n), read(k);
    for(int i = 1;i<=n;++i) read(h[i]), psa[i] = psa[i - 1] + h[i];
    for(int i = 1;i<=n;++i) {
        a[i] = h[i];
        deque<int> tmp;
        while(!stk.empty() && gcd(a[stk.back()], a[i]) != a[stk.back()]) {
            tmp.pf(stk.back());
            a[stk.back()] = gcd(a[stk.back()], a[i]);
            stk.pop_back();
        }
        tmp.pb(i);
        for(auto x : tmp) if(stk.empty() || a[x] != a[stk.back()]) stk.pb(x);
        for(auto x : stk) if(i - x + 1 >= k) {
            ckmax(ans, a[x] * (psa[i] - psa[x - 1]));
        }
    }
    print(ans, '\n');
}
