...
const int N = 1e6 + 5;
int l[N], r[N];
int dp[N], val[2][N], dep[N], tot;
bool flag = 1;

void dfs(int u = 1, int fa = 0) {
    dep[u] = dep[fa] + 1;
    if(l[u] == 0 && r[u] == 0) {
        val[0][u] = val[1][u] = dep[u];
        return;
    }
    dfs(l[u], u);
    dfs(r[u], u);
    if(val[0][l[u]] >= val[1][r[u]] && abs(val[0][l[u]] - val[1][r[u]]) < 2) dp[u] = dp[l[u]] + dp[r[u]];
    else if(val[1][l[u]] <= val[0][r[u]] && abs(val[1][l[u]] - val[0][r[u]]) < 2) dp[u] = dp[l[u]] + dp[r[u]] + 1;
    else flag = 0;
    val[0][u] = min(val[0][l[u]], val[0][r[u]]);
    val[1][u] = max(val[1][l[u]], val[1][r[u]]);
    flag &= val[1][u] - val[0][u] < 2;
}
