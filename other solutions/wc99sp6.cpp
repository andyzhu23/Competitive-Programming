...
struct cmp {
    bool operator()(const pii& u, const pii& v) {
        return a[u.fir][u.sec] > a[v.fir][v.sec];
    }
};

priority_queue<pii, vpii, cmp> heap;
...
inline void solve(){
    ...
    int ans = 0;
    while(!heap.empty()) {
        auto[u, v] = heap.top(); heap.pop();
        for(int i = 0;i<4;++i) {
            int uu = u + dir[i][0];
            int vv = v + dir[i][1];
            if(uu > 0 && uu <= n && vv > 0 && vv <= m && !vis[uu][vv]) {
                ans += max(0, a[u][v] - a[uu][vv]);
                a[uu][vv] = max(a[uu][vv], a[u][v]);
                vis[uu][vv] = 1;
                heap.push({uu, vv});
            }
        }
