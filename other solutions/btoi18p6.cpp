...
int n, m, k, a[N], cnt[N][8];
vi e[N];
ll ans;

void get2() {
    for(int u = 1;u<=n;++u) {
        for(auto v : e[u]) ans += a[v] != a[u];
    }
}

void get3() {
    for(int u = 1;u<=n;++u) {
        for(int i = 1;i<=k;++i) if (i != a[u]) {
            for(int j = 1;j<=k;++j) if(i != j && j != a[u]) {
                ans += 1ll * cnt[u][i] * cnt[u][j];
            }
        }
    }
}
...
void get4() {
    for(int u = 1;u<=n;++u) {
        for(int v : e[u]) if(a[u] != a[v]) {
            for(int i = 1;i<=k;++i) if (i != a[u] && i != a[v]) {
                for(int j = 1;j<=k;++j) if(i != j && j != a[v] && j != a[u]) {
                    ans += 1ll * cnt[u][i] * cnt[v][j];
                }
            }
        }
    }
...
void get5() {
    for(int u = 1;u<=n;++u) {
        ...
        for(int v : e[u]) if(a[u] != a[v]) {
            for(int i = 1;i<=k;++i) if(i != a[u] && i != a[v]) {
                tmp[min(i, a[v])][max(i, a[v])] += cnt[v][i];
            }
        }
        for(int v : e[u]) if(a[u] != a[v])  {
            for(int i = 1;i<=k;++i) if(i != a[u] && i != a[v]) {
            ...
            for(int i = 1;i<=k;++i) if(i != a[u] && i != a[v]) {
                for(int j = i + 1;j<=k;++j) if(j != a[u] && j != a[v]) {
                    for(int l = 1;l<=k;++l) if(l != a[u] && l != a[v] && l != i && l != j) {
                        ans += 1ll * tmp[i][j] * cnt[v][l];
                    }
                }
            }
            for(int i = 1;i<=k;++i) if(i != a[u] && i != a[v]) {
