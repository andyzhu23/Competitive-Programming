...
const int N = 2005;
int n, p, q, a[N];
// dp[i][j]: ith position, used j small camera, minimum number of big camera
int dp[N][N];
...
bool ok(int x) {
    ...
    for(int i = 1;i<=n;++i) {
        // use small camera
        while(a[l1 + 1] <= a[i] - x) ++l1;
        for(int j = 1;j<=p;++j) {
            ckmin(dp[i][j], dp[l1][j - 1]);
        }
        // use big camera
        while(a[l2 + 1] <= a[i] - x * 2) ++l2;
        for(int j = 0;j<=p;++j) {
            ckmin(dp[i][j], dp[l2][j] + 1);
        }
    }
    for(int j = 0;j<=p;++j) if(dp[n][j] <= q) return 1;
    return 0;
}
