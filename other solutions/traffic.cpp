...
inline void solve(){
    ...
    auto get = [&](int u, int t) {
        int x = t / (g[u] + r[u]);
        if(g[u] == 0 || r[u] == 0) return t;
        if(t % (g[u] + r[u]) < g[u]) return t;
        else return (x + 1) * (g[u] + r[u]);
    };
    while(!heap.empty()) {
        int u = heap.top(); heap.pop();
        // cout<<u<<' '<<dist[u]<<'\n';
        for(auto[v, w] : e[u]) {
            int tmp = get(v, dist[u] + w);
            if(dist[v] > tmp) {
                dist[v] = tmp;
                heap.push(v);
            }
