...
int id(int x, int y, int z) {
    return x * (n * m) + (y - 1) * m + z - 1; 
}

bool ok(int ii, int jj, int i, int j) {
    if(i > 0 && i <= n && j > 0 && j <= m) {
        bool flag = 1;
        for(auto[a, b, c, d, g] : v) {
            if(a == ii && b == jj && c == i && d == j) flag = 0;
            if(a == i && b == j && c == ii && d == jj) flag = 0;
        }
        return flag;
    }
    return 0;
...
inline void solve(){
    ...
    read(t);
    for(int i = 1;i<=t;++i) {
        int a = read(), b = read(), c = read(), d = read(), g = read();
        v.pb({a, b, c, d, g - 1});
    }
    for(int i = 1;i<=n;++i) {
        for(int j = 1;j<=m;++j) {
    ...
    for(int i = 0;i<(1 << k);++i) {
        for(auto[a, b, c, d, g] : ee) {
            e[id(i, a, b)].pb({id(i, c, d), 1});
        }
        for(auto[a, b, c, d, g] : v) {
            if(g == -1) continue;
            if(i >> g & 1) {
                e[id(i, a, b)].pb({id(i, c, d), 1});
                e[id(i, c, d)].pb({id(i, a, b), 1});
            }
        }
        for(auto[a, b, c, d, g] : key) if(i >> c & 1) {
            int j = i ^ (1 << c);
            e[id(j, a, b)].pb({id(i, a, b), 0});
        }
    }
