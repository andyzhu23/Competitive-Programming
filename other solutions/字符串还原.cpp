...
inline void solve(){
    read(n);
    scanf("%s\n%s\n%s\n", c[0] + 1, c[1] + 1, c[2] + 1);
    for(int i = 1;i<=n;++i) for(int j = 0;j<3;++j) c[j][i] -= 'a' - 1;
    for(int i = 0;i<3;++i) {
        for(int j = i + 1;j<3;++j) {
            us<int> cnt;
            for(int k = 1;k<=n;++k) {
                cnt.ins((c[i][k] + 26 - c[j][k]) % 26);
            }
            if(cnt.size() == 1 && *cnt.begin() % 2 == 0) {
                for(int k = 0;k<3;++k) if(k != i && k != j) {
                    bool flag = 1;
                    int d = (c[k][n] - c[i][1] + 26) % 26;
                    int e = (c[j][1] - c[k][n] + 26) % 26;
                    flag = d == e;
                    if(flag) {
                        for(int u = n;u;--u) putchar(c[k][u] + 'a' - 1);
                        putchar('\n');
                        return;
                    }
                }
