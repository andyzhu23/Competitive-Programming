...
const int N = 1e4 + 5;
int n, q1, q2, t, ans[N], cnt, cnt2, Map[N];
bool flag;
vpii bst;
vi shit[N];
...
void merge(vector<int>& array, int const left, int const mid, int const right)
{
    ...
    while (indexOfSubArrayOne < subArrayOne && indexOfSubArrayTwo < subArrayTwo) {
        if (get(leftArray[indexOfSubArrayOne], rightArray[indexOfSubArrayTwo])) {
...
inline void solve(){
    ...
    for(int i = q2 + 1;i<=n;++i) {
        int l = 0, r = bst.size() - 1, ans = 0;
        while(l <= r) {
            int mid = l + r >> 1;
            if(get(bst[mid].sec, i)) {
                l = mid + 1;
                ans = mid;
            } else r = mid - 1;
        }
        // cout<<"fuck "<<bst[ans].sec<<'\n';
        shit[bst[ans].fir].pb(i);
    }
    for(int i = 0;i<=n;++i) {
        mergeSort(shit[i], 0, shit[i].size() - 1);
        // sort(all(shit[i]), [&](int& a, int& b) {
        //     return get(a, b);
        // });
        int len = i;
        for(auto x : shit[i]) {
