...
const int N = 1e6 + 5;
int n, a[N], mx[N][22], mn[N][22];
ll b, t;

ll get(int x, int y) {
    int l = 31 - __builtin_clz(y - x + 1);
    return (ll)max(mx[x][l], mx[y - (1 << l) + 1][l]) - min(mn[x][l], mn[y - (1 << l) + 1][l]);
}
...
inline void solve(){
    read(n), read(b), read(t);
    for(int i = 1;i<=n;++i) {
        read(a[i]);
        mx[i][0] = mn[i][0] = a[i];
    }
    for(int l = 1;l<= 31 - __builtin_clz(n);++l) {
