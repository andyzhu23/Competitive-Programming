...
struct DSU {
    ...
    bool Union(int u, int v) {
        ...
        for(int i : nodes[fv]) nodes[fu].pb(i);
        return 1;
    }
};
...
inline void solve(){
    ...
    DSU dsu(n);
    while(dsu.com.size() > 1) {
        for(int i = 0;i<dsu.com.size() - 1;i++) {
            int a = dsu.com[i];
            int b = dsu.com[i + 1];
            cout<<"? "<<dsu.nodes[a].size()<<' '<<dsu.nodes[b].size()<<'\n';
            for(int i = 0;i<dsu.nodes[a].size() - 1;i++) cout<<dsu.nodes[a][i]<<' ';
            cout<<dsu.nodes[a][dsu.nodes[a].size() - 1]<<'\n';
            for(int i = 0;i<dsu.nodes[b].size() - 1;i++) cout<<dsu.nodes[b][i]<<' ';
            cout<<dsu.nodes[b][dsu.nodes[b].size() - 1]<<'\n';
            cout<<flush;
            int r; cin>>r;
            for(int i = 1;i<=r;i++) {
                int x; cin>>x;
