...
inline void solve(){
    ...
    ll cnt = 1;
    for(int i = 1;i<=n;i++){
        sum[i + n] = sum[i - 1 + n] + v[i];
        ll ans = abs(sum[n + i - 1] - sum[cnt] - sum[cnt] + sum[i - 1]);
        while(ans >= abs(sum[n + i - 1] - sum[cnt + 1] - sum[cnt + 1]+ sum[i - 1])){
            ans = abs(sum[n + i - 1] - sum[cnt + 1] - sum[cnt + 1]+ sum[i - 1]);
            cnt++;
        }
        if(i <n)
            cout<<ans<<" ";
