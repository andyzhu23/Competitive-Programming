...
const int N = 2e5 + 5;
int n, root, tot, dfn[N], sz[N], ans[N];
vi e[N];
...
struct fenwick {
    ...
    int query(int a) {
        int ans = 0;
        for(int i = a;i;i-=i&(-i)) ans = (ans + c[i]) % mod;
        return ans;
    }
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        int x = read();
        ans[x] = (fen.query(dfn[x], dfn[x] + sz[x] - 1) + 1) % mod;
        fen.update(dfn[x], ans[x] - fen.query(dfn[x], dfn[x]));
    }
