...
void get(int l = 1, int r = n) {
    ...
    for(int i = mid + 1;i<=r;++i) {
        while(j <= mid && a[j].b < a[i].b) fen.update(a[j++].a, 1);
        ans[a[i].c] += fen.query(a[i].a + 1, N - 1);
    }
    for(int i = l;i<j;++i) fen.update(a[i].a, -1);
    j = mid;
    for(int i = r;i>mid;--i) {
        while(j >= l && a[j].b > a[i].b) fen.update(a[j--].a, 1);
        ans[a[i].c] += fen.query(a[i].a - 1);
    }
    for(int i = j + 1;i<=mid;++i) fen.update(a[i].a, -1);
}
