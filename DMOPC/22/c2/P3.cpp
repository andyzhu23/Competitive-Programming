...
inline void solve(){
    ...
    for(int i = 2;i<=n;++i) {
        if(p[i].empty()) {
            p[i].pb(i);
            for(int j = i + i;j<=n;j += i) p[j].pb(i);
        }
    }
    for(int i = 1;i<=n;++i) {
        for(auto x : p[a[i]]) if(prv[x]) e[prv[x]].pb(a[i]), ++in[a[i]];
        for(auto x : p[a[i]]) prv[x] = a[i];
    }
    pq<int> heap;
    for(int i = 1;i<=n;++i) if(in[i] == 0) heap.push(i);
    vi ans;
    while(!heap.empty()) {
