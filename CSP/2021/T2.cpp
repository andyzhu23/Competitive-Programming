...
inline void solve(){
    ...
    for(int l = 2;l<=n;++l) {
        for(int i = 1;i + l - 1<=n;++i) {
            int j = i + l - 1;
            if((c[i] == '(' || c[i] == '?') && (c[j] == ')' || c[j] == '?')) {
                A[i][j] = (dp[i + 1][j - 1] + SA[i + 1][j - 1] + AS[i + 1][j - 1] + S[i + 1][j - 1]) % mod;
            }
            dp[i][j] = A[i][j];
            for(int kk = i;kk<j;++kk) {
                AS[i][j] = (AS[i][j] + dp[i][kk] * S[kk + 1][j]) % mod;
                SA[i][j] = (SA[i][j] + S[i][kk] * dp[kk + 1][j]) % mod;
                dp[i][j] = (dp[i][j] + dp[i][kk] * A[kk + 1][j]) % mod;
                dp[i][j] = (dp[i][j] + AS[i][kk] * A[kk + 1][j]) % mod;
            }
        }
    }
    print(dp[1][n], '\n');
