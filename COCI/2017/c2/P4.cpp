...
ordered_multiset<ll> bst;

inline void solve(){
    read(n), read(k);
    int mid = n >> 1;
    for(int i = 1;i<=n;++i) read(h[i]), read(g[i]);
    l = 1, r = mid;
    dfs();
    l = mid + 1, r = n;
    dfs2();
    ...
    for(int i = 0;i<y.size();++i) {
        while(j < x.size() && x[j].fir <= y[i].fir) {
            bst.ins(x[j].sec);
            ++j;
        }
        ans += bst.size() - bst.order_of_key(k - y[i].sec);
    }
    print(ans, '\n');
}
