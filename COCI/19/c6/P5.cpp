...
pll get(string& s) {
    ...
    for(int i = 0;i<s.size();++i) {
        if(i < s.size() - 1) ans.fir = (ans.fir * 131 + s[i]) % mod;
        if(i) ans.sec = (ans.sec * 131 + s[i]) % mod;
    }
    return ans;
}
...
ll hsh(string& s) {
    ll ans = 0;
    for(auto x : s) ans = (ans * 131 + x) % mod;
    return ans;
}
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        for(int j = 1;j<=k;++j) {
            ...
            if(i == 1) ++dp[i & 1][ID];
            pll key = get(s);
            dp[i & 1][ID] = (dp[i & 1][ID] + dp[i & 1 ^ 1][key.fir]) % mod;
            if(key.fir != key.sec) dp[i & 1][ID] = (dp[i & 1][ID] + dp[i & 1 ^ 1][key.sec]) % mod;
        }
        dp[i & 1 ^ 1].clear();
    }
    for(auto[u, v] : dp[n & 1]) ans = (ans + v) % mod;
    print(ans, '\n');
}
