...
struct segtree {
    ...
    void update(int& rt, int l, int r, int pos, int val) {
        ...
        int mid = l + r >> 1;
        if(pos <= mid) update(L[rt], l, mid, pos, val);
        else update(R[rt], mid + 1, r, pos, val);
        st[rt] = st[L[rt]] + st[R[rt]];
    }
    int query(int& rt, int l, int r, ll t) {
        if(rt == 0) rt = ++tot;
        if(l == r) return l;
        int mid = l + r >> 1;
        if(st[L[rt]] >= t) return query(L[rt], l, mid, t);
        else return query(R[rt], mid + 1, r, t - st[L[rt]]);
    }
} st;
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        int v = read(), a = read();
        tot += a;
        st.update(st.root, 1, 1e9, v, a);
        print(st.query(st.root, 1, 1e9, (tot - 1) / 2 + 1), '\n');
    }
}
