...
struct segtree {
    ...
    void push_down(int rt) {
        ckmax(st[lc].y, lazy[rt]);
        ckmax(st[rc].y, lazy[rt]);
        ckmax(mx[lc], lazy[rt]);
        ckmax(mx[rc], lazy[rt]);
        ckmax(lazy[lc], lazy[rt]);
        ckmax(lazy[rc], lazy[rt]);
        lazy[rt] = 0;
    }
    ...
    int query(int rt, int l, int r, int val) {
        if(l == r) return mx[rt] > val ? inf : l;
        int mid = l + r >> 1;
        push_down(rt);
        if(mx[rc] > val) return query(rc, mid + 1, r, val);
        else return min(mid + 1, query(lc, l, mid, val));
    }
} st;
...
inline void solve(){
    ...
    ll ansa = inf, ansb = inf, ansc = inf;
    for(auto[x, y, z] : a) {
        auto[u, v] = st.st[1];
        if(x + u + v < ansa + ansb + ansc) {
            ansa = x;
            ansb = u;
            ansc = v;
        }
        if(x) {
            int l = st.query(1, 0, 5e5, z);
            if(l < y) st.update(1, 0, 5e5, l, y - 1, z);
        }
    }
    print(ansa, ' ');
    print(ansb, ' ');
