...
vi p;
int a[N];
ll psa[N], xx[N], psxx[N], psaa[N];
ll ans;

inline void solve() {
    read(n), read(m);
    for(int i = 1;i<=m;++i) p.pb(read());
    sort(all(p));
    memset(xx, 0x3f, sizeof xx);
    for(int i = 2;i<=n;++i) read(a[i]), psa[i] = (psa[i - 1] + a[i]) % mod;
    for(int i = 1;i<=n;++i) {
        auto x = lower_bound(all(p), i);
        if(x != p.begin()) xx[i] = psa[i] - psa[*prev(x)];
        if(x != p.end()) ckmin(xx[i], psa[*x] - psa[i]);
        if(x != p.end() && *x == i) xx[i] = 0;
        psxx[i] = (psxx[i - 1] + xx[i]) % mod;
        psaa[i] = (psaa[i - 1] + psa[i]) % mod;
    }
    int pos = 0;
    for(int i = 2;i<=n;++i) {
        while(xx[pos + 1] + xx[i] < psa[i] - psa[pos + 1]) ++pos;
        ans = (ans + xx[i] * pos % mod + psxx[pos] + (i - pos - 1) * psa[i] % mod - psaa[i - 1] + mod + psaa[pos]) % mod;
    }
    print(ans, '\n');
}
