...
const int N = 2e3 + 10;
int n, m, s, f, l, r, k[N], ans[N][N];

inline void solve(){
    n = read(), m = read(), s = read(), f = read(), l = read(), r = read();
    bool ok = 0;
    for(int i = 1;i<=m;++i) k[i] = read(), ok |= k[i] == f;
    if(s == f && (!ok || r == 0)) {
        if(n * m - m < l) {
            puts("-1");
            return;
    ...
    if(l == 0) {
        ...
        for(int j = 1;j<=m;++j) {
            bool flag2 = 1;
            for(int i = 1;i<=n;++i) {
                if(i == f && j > pos) ans[i][j] = 0;
                else if(i == cur && j == pos && flag2) {
                    flag = 1, ans[i][j] = 1, cur = f, --l, flag2 = 0;
                }
                else if(!flag && l > 1) {
                    ans[i][j] = 1, --l;
                    if(cur == i && flag2) cur = k[j], flag2 = 0;
                } else if(flag && l > 0) {
                    ans[i][j] = 1, --l; 
                    if(cur == i && flag2) cur = k[j], flag2 = 0;
                } else ans[i][j] = 0;
