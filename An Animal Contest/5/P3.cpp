...
void dfs(pii seg, int u = 1, int fa = 0) {
    ans[u] = seg.sec - seg.fir + 1;
    sort(all(e[u]));
    int del = -1;
    for(int i = 0;i<e[u].size();++i) {
        int v = e[u][i].sec;
        if(v == fa) del = e[u][i].fir;
    }
    if(u != 1) e[u].erase(lb(all(e[u]), (pii){del, fa}));
    for(int i = 0;i<e[u].size();++i) {
        int v = e[u][i].sec;
        if(v == fa) continue;
        int d = e[u][i].fir;
        int last, nx;
        if(i == 0) last = -INF;
        else last = e[u][i - 1].fir;
        if(i == e[u].size() - 1) nx = INF;
        else nx = e[u][i + 1].fir;
        // cout<<v<<' '<<ub(s + seg.fir, s + seg.sec + 1, (d + last) / 2) - s<<' '<<ub(s + seg.fir, s + seg.sec + 1, (d + nx) / 2) - s - 1<<'\n';
        pii nxt = {ub(s + seg.fir, s + seg.sec + 1, (d + last) / 2) - s, ub(s + seg.fir, s + seg.sec + 1, (d + nx) / 2) - s - 1};
        dfs(nxt, v, u);
    }
}
