...
public:
    ...
    void update(int a, ll b) {
        for(int i = a;i<=m;i += lowbit(i)) c[i] += b;
    }
    ll query(int a) {
        ll ans = 0;
    ...
    ll query(int a, int b) {
        if(a > b) return 0;
        return max((ll)0, query(b) - query(a - 1));
    }
} fen, fen2;
...
inline void solve(){
    ...
    for(int i = 1;i<=v;++i) {
        ...
        dy.ins(b);
        dy.ins(c);
        vv[i] = {a, min(b, c), max(b, c)};
    }
    for(auto x : dx) mpx[x] = ++n;
    for(auto x : dy) mpy[x] = ++m;
    for(int i = 1;i<=h;++i) {
        line cur = hh[i];
        hh[i] = {mpy[cur.a], mpx[cur.b], mpx[cur.c]};
    }
    for(int i = 1;i<=v;++i) {
        line cur = vv[i];
        vv[i] = {mpx[cur.a], mpy[cur.b], mpy[cur.c]};
    }
    sort(vv + 1, vv + v + 1, [&](line& a, line& b) {
        return a.a < b.a;
    });
    sort(hh + 1, hh + h + 1, [&](line& a, line& b) {
        return a.c > b.c;
    });
    ll ans = 0;
    int j = 0;
    ...
    for(int i = v;i;--i) {
        fen.init();
        fen2.init();
        while(j < h && hh[j + 1].c >= vv[i].a) opt.ins(hh[++j]);
        auto it = opt.begin();
        for(int k = 1;k<i;++k) if(vv[i].a != vv[k].a) {
            for(;it != opt.end() && (*it).b <= vv[k].a; ++it) {
                ll tmp = fen.query((*it).a, (*it).a);
                fen.update((*it).a, 1);
                fen2.update((*it).a, tmp);
            }
            ll tmp = fen.query(max(vv[i].b, vv[k].b), min(vv[i].c, vv[k].c));
            ans += (tmp * (tmp - 1) >> 1) - fen2.query(max(vv[i].b, vv[k].b), min(vv[i].c, vv[k].c));
        } else break;
    }
    print(ans, '\n');
}
