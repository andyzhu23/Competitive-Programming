...
bool ok(int mid) {
    ...
    for(int i = mid;i>0;--i) {
        if(pos + i <= n && pos - i > 0) {
            if(min(abs(pos + i - n / 2), abs(pos + i - (n - 1) / 2 - 1)) > 
                min(abs(pos - i - n / 2), abs(pos - i - (n - 1) / 2 - 1))) pos -= i, a.pb(-i);
            else pos += i, a.pb(i);
        }
        else if(pos + i <= n) pos += i, a.pb(i);
        else if(pos - i > 0) pos -= i, a.pb(-i);
        else return 1;
    }
    if(pos + mid + 1 > n && pos - mid - 1 <= 0) return 1;
    return 0;
}
