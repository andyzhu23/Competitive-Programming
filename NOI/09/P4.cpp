...
vi a[NM];
bitset<NM> vis, mk;
int tot, cnt, head[NM], dist[NM], cur[NM];

struct edge {int u, v, f, nxt;} e[1000005];
...
int dfs(int u = S, int f = inf) {
    ...
    for(int& i = cur[u];i;i=e[i].nxt) {
        if(!vis[e[i].v] && dist[e[i].v] == dist[u] + 1 && e[i].f) {
