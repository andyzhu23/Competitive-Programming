...
//--------------------- start of program ---------------------
const int N = 3004;
int r, c, h, w, a[N][N], sum[N][N], ans = INF;
...
bool ok(int x) {
    ...
    for(int i = 1;i<=r;++i) {
        for(int j = 1;j<=c;++j) {
            ...
            else if(a[i][j] < x) sum[i][j] = 1;
        }
    }
    for(int i = 1;i<=r;++i) {
        for(int j = 1;j<=c;++j) sum[i][j] += sum[i - 1][j] + sum[i][j - 1] - sum[i - 1][j - 1];
    }
    bool flag = 0;
    for(int i = 1;i<=r - h + 1;++i) {
        for(int j = 1;j<=c - w + 1;++j) {
            int tot = get(i, j, i + h - 1, j + w - 1);
            if(tot == 0) ckmin(ans, x);
            flag |= tot >= 0;
        }
    }
    return flag;
