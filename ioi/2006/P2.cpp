...
struct fenwick {
    ...
    void update(int i, node x) {
        while(i <= n) {
            d[i] = c[i];
            for(int j = 1;j<lowbit(i);j<<=1) ckmin(d[i], d[i - j]);
            i += lowbit(i);
        }
    }
    void init(vec<node>& a) {
    ...
    node query(int x, int y) {
        ...
        while(y >= x) {
            ckmin(ans, c[y]);
            --y;
            for(;y-lowbit(y)>=x;y-=lowbit(y)) ckmin(ans, d[y]);
        }
        return ans;
    }
} st[N], tmp;
...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        arr[i] = vector<node>(N);
        for(int j = 1;j<=m;++j) {
            psa[i][j] = psa[i - 1][j] + psa[i][j - 1] + read() - psa[i - 1][j - 1];
        }
    }
    for(int i = 1;i + c - 1<=n;++i) {
        for(int j = 1;j + d - 1<=m;++j) {
            arr[i][j].val = psa[i + c - 1][j + d - 1] - psa[i - 1][j + d - 1] - psa[i + c - 1][j - 1] + psa[i - 1][j - 1];
            arr[i][j].x = i;
            arr[i][j].y = j;
        }
    }
    vi ans(5);
    for(int i = 1;i<=n;++i) st[i].init(arr[i]);
    for(int j = 1;j + b - 1<=m;++j) {
        for(int i = 1;i<=n;++i) {
            e[i] = st[i].query(j + 1, j + b - 1 - d);
        }
        tmp.init(e);
        for(int i = 1;i + a - 1<=n;++i) {
            node ret = tmp.query(i + 1, i + a - 1 - c);
            int tot = psa[i + a - 1][j + b - 1] - psa[i - 1][j + b - 1] - psa[i + a - 1][j - 1] + psa[i - 1][j - 1];
            if(tot - ret.val > ans[0]) {
                ans[0] = tot - ret.val;
                ans[1] = j;
                ans[2] = i;
                ans[3] = ret.y;
