...
inline void solve(){
    ...
    for(int i = 1;i<=n;++i) {
        int mx = -1;
        for(int j = 1;j<=n;++j) {
            if((mx == -1 || dist[j] > dist[mx]) && !vis[j]) mx = j;
        }
        ans += dist[mx];
        vis[mx] = 1;
        for(int j = 1;j<=n;++j) {
            ckmax(dist[j], max({(a[mx] + a[j]) % m, abs(a[mx] - a[j]) % m, a[mx] * a[j] % m, max(a[mx], a[j]) % min(a[mx], a[j]) == 0 ? max(a[mx], a[j]) / min(a[mx], a[j]) % m : 0}));
        }
    }
    print(ans, '\n');
}
