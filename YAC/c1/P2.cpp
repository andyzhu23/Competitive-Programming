...
inline void solve(){
    ...
    if(n & 1) {
        ...
        out(nx, ny);
        for(int i = 0;i<4;++i) {
            if(nx + dir[i][0] <= 0 || nx + dir[i][0] > n || ny + dir[i][1] <= 0 || ny + dir[i][1] > n) continue;
            if(a[nx + dir[i][0]][ny + dir[i][1]] == 0) {
                a[nx + dir[i][0]][ny + dir[i][1]] = 2;
            }
        }
        a[nx][ny] = 1;
    }
