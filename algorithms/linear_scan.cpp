#include <bits/stdc++.h>
using namespace std;
using ll = long long;
const int inf = 0x3f3f3f3f;
const int N = 1e4 + 5;
int n, m, k;
double rnk[N];

map<double, int> mp;
map<double, vector<pair<pair<double, double>, int> > > b;
int tot;

struct segtree {
    double st[N << 2];
    int lazy[N << 2];
#define lc (rt << 1)
#define rc (rt << 1 | 1)
    inline void upd(int rt, int l, int r, int x, int y, int val) {
        if(l == x && y == r) {
            lazy[rt] += val;
            if(lazy[rt]) st[rt] = rnk[r] - rnk[l - 1];
            else st[rt] = st[lc] + st[rc];
            return;
        }
        int mid = (l + r) >> 1;
        if(y <= mid) upd(lc, l, mid, x, y, val);
        else if(x > mid) upd(rc, mid + 1, r, x, y, val);
        else upd(lc, l, mid, x, mid, val), upd(rc, mid + 1, r, mid + 1, y, val);
        if(lazy[rt]) st[rt] = rnk[r] - rnk[l - 1];
        else st[rt] = st[lc] + st[rc];
    }
    inline double query(int rt, int l, int r, int x, int y) {
        if(l == x && y == r) return st[rt];
        int mid = (l + r) >> 1;
        if(y <= mid) return query(lc, l, mid, x, y);
        else if(x > mid) return query(rc, mid + 1, r, x, y);
        else return query(lc, l, mid, x, mid) + query(rc, mid + 1, r, mid + 1, y);
    }
    inline void clear() {
        memset(st, 0, sizeof st);
        memset(lazy, 0, sizeof lazy);
    }
} st;
set<double> dis;

int main() {
    cin.tie(0)->ios::sync_with_stdio(0);
#ifdef LOCAL
    freopen("in.txt", "r", stdin);
    freopen("out.txt", "w", stdout);
#endif
    cin>>k;
    for(int i = 1;i<=k;++i) {
        double aa, bb, cc, dd;
        cin>>aa>>bb>>cc>>dd;
        dis.insert(bb);
        dis.insert(bb - 0.00000000001);
        dis.insert(dd);
        b[aa].push_back({{bb, dd}, 1});
        b[cc + 0.00000000001].push_back({{bb, dd}, -1});
    }
    for(auto x : dis) mp[x] = ++tot, rnk[tot] = x;
    dis.clear();
    double ans = 0;
    b[inf];
    for(auto[u, v] : b) if(u < inf) {
        for(auto[x, y] : v) {
            int l = mp[x.first];
            int r = mp[x.second];
            st.upd(1, 1, tot, l, r, y);
        }
        ans += 1ll * st.query(1, 1, tot, 1, tot) * ((*b.upper_bound(u)).first - u);
    }
    printf("%.2lf\n", ans);
    return 0;
}
