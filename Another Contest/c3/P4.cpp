...
struct segtree2 {
    ll st[N << 2], lazy[N << 2];
    void push_down(int rt, int l, int r, int mid) {
        st[lc] += 1ll * (l + mid) * (mid - l + 1) / 2 * lazy[rt];
        st[rc] += 1ll * (r + mid + 1) * (r - mid) / 2 * lazy[rt];
        lazy[lc] += lazy[rt];
        lazy[rc] += lazy[rt];
        lazy[rt] = 0;
    }
...
inline void solve(){
    ...
    while(q--) {
        ...
        if(opt == 1) {
            ...
            st1.update(1, 1, n, l, r, (1 - l) * x);
            st2.update(1, 1, n, l, r, x);
        } else {
            int l = read(), r = read();
            print(st1.query(1, 1, n, l, r) + st2.query(1, 1, n, l, r), '\n');
        }
    }
}
