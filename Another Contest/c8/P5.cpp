...
struct fenwick {
    ...
    void update(int a, T b) {
        for(int i = a; i<=n;i+=lowbit(i)) st[i] = (st[i] + b) % MOD;
    }
    T query(int a) {
        T ans = 0;
        for(int i = a;i;i-=lowbit(i)) ans = (ans + st[i]) % MOD;
        return ans;
    }
};
...
inline void solve(){
    int n; cin>>n;
    fenwick<ll> st1(n), st2(n);
    vll a(n + 1), b(n + 1), c(n + 1);
    for(int i = 1;i<=n;i++) cin>>a[i];
    for(int i = 1;i<=n;i++) {
        b[i] = st1.query(a[i] - 1) + 1;
        st1.update(a[i], b[i]);
    }
    for(int i = n;i;i--) {
        c[i] = st2.query(a[i] - 1) + 1;
        st2.update(a[i], c[i]);
    }
    ll ans = 0;
    for(int i = 1;i<=n;i++)
